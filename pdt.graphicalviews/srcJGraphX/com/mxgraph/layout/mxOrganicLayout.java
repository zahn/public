...
public class mxOrganicLayout extends mxGraphLayout {
	 ...
	 */
	protected double averageNodeArea = 16000; // 160000;
	// reduced to avoid empty space on top of the graph.
	// TODO avoid empty space left from the graph
	...
	public void execute(Object parent) {
		...
		for (int i = 0; i < vertices.length; i++) {
			Object[] edges = mxGraphModel.getEdges(model, vertices[i], false,
					true, false);
			...
			for (int j = 0; j < edges.length; j++) {
				...
				if (view.getVisibleTerminal(edges[j], true) == vertices[i]
						&& vertexSet.contains(view.getVisibleTerminal(edges[j],
								false))) {
		...
		if (averageNodeArea == 0.0) {
			...
			if (bounds != null) {
				double centreX = totalBounds.getX() + totalBounds.getWidth()
						/ 2.0;
				double centreY = totalBounds.getY() + totalBounds.getHeight()
						/ 2.0;
